package com.bozhong.lhdataaccess.client.common.constants.util;
...
//    }
     ...
     * @return 返回加1的字符串
     */
    public static String contains4(String str){

        //把最大的code加1
        int temp = Integer.parseInt(str);
        temp++;
        //判断加1之后的code是否包含数字4，直到找到不含4的字符串

        String code = String.valueOf(temp);
        while(code.contains("4")){
    ...
    /**
     * 补齐apk的版本号
     * @return 例：v2.6.5补齐到v2.6.5.0
     */
    public static String polishingApkVersion(String version) {
        if (isBlank(version)) {
            return version;
        }
     ...
     * @return 例: .txt  .apk
     */
    public static String subStringExpandedName(String filePath) {
        return filePath.substring(filePath.lastIndexOf("."), filePath.length());
    }

    /**
     * 转化成key value的json 例：[{"mobile":"11293838483"},{"mobile":"17302828192"}]
     * @param key
     * @param value
     * @return
     */
    public static String convertToJson(String key, String[] value) {
        if (isBlank(key) || value == null || value.length <= 0) {
            return null;
        }
        StringBuilder stringBuilder = new StringBuilder();
        for (String mobile : value) {
            String receive = "{\"" + key + "\":\"" + mobile + "\"},";
     ...
     * @return
     */
    public static String convertToJson(String key, List<String> value) {
        if (isBlank(key) || value == null || value.size() <= 0) {
            return null;
        }
        StringBuilder stringBuilder = new StringBuilder();
        for (String mobile : value) {
