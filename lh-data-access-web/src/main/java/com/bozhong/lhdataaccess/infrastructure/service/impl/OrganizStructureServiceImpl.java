...
public class OrganizStructureServiceImpl implements OrganizStructureService {
    ...
    public void dumpOrganizStructureData(Date lastUpdateTime) {
        ...
        while(true){
            ...
            //避免死循环
            if(pageIndex>500){
                logger.error("组织机构数据dump的时候，循环超出了范围，时间是"+lastUpdateTime);
                break;
            }
            try{
                //调用中兴接口获取组织机构代码数据
                ...
                Map resultMap = JSONObject.fromObject(HttpUtil.httpPostWithJson("http://118.178.131.147:6003/hospital/personDetails/organizationDetail", attributes));
                Object code = resultMap.get("code");
                if(code==null || !"200".equals(code.toString())){
                    logger.error("组织架构数据dump的时候，调用接口出错，code = "+code+",msg = "+(String)resultMap.get("msg"));
                ...
                for(int i=0;i<backBodyJson.size();i++){
                    ...
                    organizStructureDO.setLastEditedTime(sdf.parse(job.get("lastEditedTime").toString()));
